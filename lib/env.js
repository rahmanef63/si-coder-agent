...
// Remove keys from the managed si-coder block. Only the block is touched — an export the
// user wrote OUTSIDE it is left alone and reported, mirroring appendExportToShellRc's SEC-1
// rule that onboarding never edits what it does not own.
// Returns { removed: [...], unmanaged: [...] }.
function removeExportsFromShellRc(keys = [], shellRcPath = path.join(os.homedir(), '.bashrc')) {
  const wanted = new Set(keys);
  if (!fs.existsSync(shellRcPath) || wanted.size === 0) return { removed: [], unmanaged: [] };
  const content = fs.readFileSync(shellRcPath, 'utf8');

  const removed = [];
  const blockRe = /(# --- si-coder onboarding[^\n]*\n)([\s\S]*?)(# --- end si-coder onboarding ---)/g;
  const next = content.replace(blockRe, (_m, head, body, tail) => {
    const kept = body.split(/\r?\n/).filter((line) => {
      const mm = /^export\s+([A-Za-z_][A-Za-z0-9_]*)=/.exec(line.trim());
      if (mm && wanted.has(mm[1])) { removed.push(mm[1]); return false; }
      return line.trim() !== '' || false;
    });
    return `${head}${kept.length ? kept.join('\n') + '\n' : ''}${tail}`;
  });

  // Anything still exported after the managed blocks are accounted for is user-owned.
  const stripped = content.replace(blockRe, '');
  const unmanaged = [];
  const re = /^export\s+([A-Za-z_][A-Za-z0-9_]*)=/gm;
  let m;
  while ((m = re.exec(stripped)) !== null) if (wanted.has(m[1])) unmanaged.push(m[1]);

  if (removed.length) {
    fs.writeFileSync(shellRcPath, next, { mode: 0o600 });
    fs.chmodSync(shellRcPath, 0o600);
  }
  return { removed, unmanaged: [...new Set(unmanaged)] };
}
