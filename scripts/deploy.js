...
// HYBRID target (VPS frontend on Dokploy + Convex Cloud backend) reuses the Cloud
// deploy helpers: push the backend to Convex Cloud, derive its public URL for the
// frontend build arg. deriveCloudUrl returns null for preview/project keys.
const { deployBackendOnly, deriveCloudUrl } = require('../lib/convex-cloud');
...
// ---- HYBRID target helpers (VPS frontend + Convex Cloud backend) ----
// This monolith covers the self-hosted 'dokploy' path and the 'hybrid' path
// (Dokploy frontend app, Convex Cloud backend). The 'vercel' path lives in
// /sc-vercel — reject it here with a pointer rather than half-running it.
const VALID_TARGETS = new Set(['dokploy', 'hybrid']);
function resolveTarget(t) {
  const target = (t || 'dokploy').toLowerCase();
  if (target === 'vercel') {
    throw new Error(
      "target 'vercel' is handled by /sc-vercel — run skills/sc-vercel/scripts/deploy.js " +
      '(this monolith covers dokploy|hybrid).',
    );
  }
  if (!VALID_TARGETS.has(target)) {
    throw new Error(`unknown --target '${t}'. Use one of: ${[...VALID_TARGETS].join(', ')}.`);
  }
  return target;
}

// Resolve the public Convex Cloud URL for a hybrid deploy. Prefer the URL derived
// from a prod deploy key (it carries the deployment name); fall back to an explicit
// env URL for preview/project keys, which don't. Returns null when neither is known.
function resolveHybridConvexUrl({ deployKey, envUrl } = {}) {
  const derived = deriveCloudUrl(deployKey);
  if (derived) return derived;
  const fallback = typeof envUrl === 'string' ? envUrl.trim() : '';
  return fallback || null;
}

// SEC-1: parse only NON-secret values from argv. Secrets (DOKPLOY_API_URL,
// DOKPLOY_API_KEY, GITHUB_TOKEN, CONVEX_DEPLOY_KEY) come EXCLUSIVELY from the
// environment so they never occupy an argv slot visible via `ps aux` /
// /proc/<pid>/cmdline. Non-secret flags: --project/--app/--domain/--target.
// Bare positionals are accepted as <PROJECT> <APP> [DOMAIN] for convenience.
function parseDeployArgs(argv = []) {
  const out = { project: undefined, app: undefined, domain: undefined, target: undefined };
  const positional = [];
  for (let i = 0; i < argv.length; i++) {
    ...
    } else if (a === '--project' || a === '--app' || a === '--domain' || a === '--target') {
      const k = a.slice(2);
...
async function run() {
  ...
  // Resolve the deploy target and, for hybrid, the Convex Cloud backend URL — fail fast
  // (mirrors the usage block above) before any GitHub/Dokploy side effects run.
  let target;
  try {
  ...
  if (target === 'hybrid') {
    if (!convexDeployKey) {
      console.error('deploy.js: --target hybrid needs CONVEX_DEPLOY_KEY in the environment (Convex Cloud backend). Run /sc-onboarding --domains convex-cloud.');
      process.exit(1);
    }
    hybridConvexUrl = resolveHybridConvexUrl({
      deployKey: convexDeployKey,
      envUrl: process.env.NEXT_PUBLIC_CONVEX_URL || process.env.CONVEX_CLOUD_URL,
    });
    if (!hybridConvexUrl) {
      console.error('deploy.js: --target hybrid could not resolve the Convex Cloud URL. A prod deploy key carries it; for a preview/project key, set NEXT_PUBLIC_CONVEX_URL (or CONVEX_CLOUD_URL) in the environment.');
      process.exit(1);
    }
  }
  ...
  try {
    ...
    // --- 1b. HYBRID: CONVEX CLOUD BACKEND ---
    // Push the backend to Convex Cloud before the Dokploy frontend so the app builds
    // against a live backend. deployBackendOnly runs `npx convex deploy` with the deploy
    // key via env (never argv). A failure here is non-fatal — the frontend still deploys,
    // but its NEXT_PUBLIC_CONVEX_URL won't answer until the backend deploy succeeds.
    if (target === 'hybrid') {
      if (fs.existsSync(path.join(process.cwd(), 'convex'))) {
        try {
          console.log('☁️ Hybrid target: deploying Convex backend to Convex Cloud...');
          deployBackendOnly({ deployKey: convexDeployKey, cwd: process.cwd() });
          console.log(`✅ Convex Cloud backend deployed. NEXT_PUBLIC_CONVEX_URL=${hybridConvexUrl}`);
        } catch (err) {
          console.warn(`⚠️ Convex Cloud backend deploy failed: ${err.message}`);
          console.warn('   Frontend will still deploy, but the app cannot reach a live backend until this succeeds.');
        }
      } else {
        console.log('ℹ️ Hybrid target but no convex/ dir found — skipping Convex Cloud backend deploy.');
      }
    }
    ...
    // Hybrid runs the backend on Convex Cloud (deployed in step 1b), so never stand up a
    // self-hosted compose — even if the repo carries a docker-compose.yml for the other path.
    if (target === 'hybrid' && hasDockerCompose) {
      console.log('\nℹ️ Hybrid target: skipping self-hosted Convex compose — backend runs on Convex Cloud.');
    }
    if (hasDockerCompose && target !== 'hybrid') {
    ...
    if (hasDockerfile) {
      ...
      if (app) {
        ...
        try {
          ...
          // image layers via env/buildArgs (it is a backend-only secret). Self-hosted
          // (dokploy) derives it from the api- subdomain; hybrid uses the Convex Cloud
          // URL resolved up front (independent of the frontend's own domain).
          let convexUrl = null;
          if (target === 'hybrid') convexUrl = hybridConvexUrl;
          else if (domain) convexUrl = `https://api-${domain}`;
          const appEnv = convexUrl ? `NEXT_PUBLIC_CONVEX_URL=${convexUrl}` : '';
