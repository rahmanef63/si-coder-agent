...
const { assertConvexResolvable } = require('./convex');
...
function deployCloud({
  ...
  // Preflight: the convex CLI must be resolvable from the target app, else `npx`
  // would fail late (or hang on an install prompt) mid-deploy. Surface it upfront.
  assertConvexResolvable(cwd);
  // `npx --yes` so a zero-human run can never stall on an "install convex?" prompt.
  const args = ['--yes', 'convex', 'deploy', '--cmd', buildCmd, '--cmd-url-env-var-name', urlEnvVar];
...
function deployBackendOnly({ deployKey, cwd = process.cwd(), message } = {}) {
  ...
  // Same preflight as deployCloud: fail fast if the convex CLI isn't installed,
  // and `npx --yes` so a non-interactive run can't hang on an install prompt.
  assertConvexResolvable(cwd);
  const args = ['--yes', 'convex', 'deploy', ...(message ? ['--message', message] : [])];
  execFileSync('npx', args, {
    env: { ...process.env, CONVEX_DEPLOY_KEY: deployKey },
    stdio: 'inherit',
    cwd,
...
function deriveCloudUrl(deployKey) {
  ...
  if (!name) return null;
  // The name is interpolated straight into a hostname, so constrain it to the
  // Convex deployment-name charset (lowercase alnum + hyphen). Anything else is
  // a malformed/garbage key — return null (same contract as the other parse
  // failures above) rather than emitting a bogus or injectable URL.
...
async function probeCloud({ deploymentUrl, timeoutMs = 10000 } = {}) {
  ...
  if (deploymentUrl) {
    ...
    // @convex-dev/auth serves JWKS on the SITE host (`<name>.convex.site`), NOT the
    // deployment/api host (`<name>.convex.cloud`). Swap a TRAILING '.convex.cloud' for
    // '.convex.site' and probe JWKS there; /version stays on the .convex.cloud host.
    // A custom (non-.convex.cloud) domain has no derivable site host, so leave jwks
    // not-checked (absent from results) rather than false-failing a healthy deploy.
    const siteUrl = deploymentUrl.endsWith('.convex.cloud')
      ? deploymentUrl.slice(0, -'.convex.cloud'.length) + '.convex.site'
      : null;
    if (siteUrl) await check('jwks', `${siteUrl}/.well-known/jwks.json`);
  }
  return results;
}
