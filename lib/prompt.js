    ...
    const onData = d => {
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        if (code === CR || code === LF || code === EOT) {
          // A paste/PTY can deliver `value\r\nnext-value\r\n` in one data event.
          // Do not discard bytes that belong to the next prompt. Treat CRLF as one Enter.
          let next = i + 1;
        ...
        if (code === 27 && escapeCancels) {
          // Esc cancels this prompt. Discard the rest of the same terminal event so an
          // escape sequence such as ESC [ A cannot leak `[A` into the next credential.
          return finish(null);
        }
        if (code === BS || code === DEL) {
          if (buf) {
