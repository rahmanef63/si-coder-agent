...
function generateAdminKey({ containerName }) {
  ...
  const raw = execSync(`sudo docker exec ${containerName} ./generate_admin_key.sh`, { stdio: 'pipe' }).toString();
  const adminKey = extractAdminKey(raw);
  if (!adminKey) throw new Error('admin-key extraction returned empty');
  return adminKey;
}

function deploySchema({ apiDomain, adminKey, cwd = process.cwd() }) {
  if (!apiDomain || !adminKey) throw new Error('apiDomain + adminKey required');
  execSync(
    `NODE_TLS_REJECT_UNAUTHORIZED=0 npx convex deploy --url https://${apiDomain} --admin-key "${adminKey}"`,
    { stdio: 'inherit', cwd },
  );
}
