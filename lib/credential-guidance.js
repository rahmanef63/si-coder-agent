...
function credentialGuide(key, { store, user } = {}) {
  ...
  const saveWith = providerId ? (user ? `sc user credential-set ${user} ${providerId} ${key}` : `sc secret set ${providerId} ${key}`) : null;
  ...
  const continueWith = providerId ? (user ? `sc user credentials ${user} ${providerId}` : `sc doctor --providers ${providerId}`) : null;
  return {
    provider: providerId,
    key,
