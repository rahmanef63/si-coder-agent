...
function listProfileRecords() {
  return listProfiles().map(name => ({ name, owner: profileOwner(name), keys: Object.keys(readProfile(name)).length }));
}
...
function duplicateProfile(source, target, { owner = target, replaceEmpty = false } = {}) {
  ...
  writeProfileMeta(meta);
  return { source, target, keys: Object.keys(copied).sort() };
}
...
function loadEnvForProfile(name, { shellRcEnv = {}, reason = 'explicit user selection' } = {}) {
  assertName(name);
  ...
  return { env, profile: name, owner: profileOwner(name), reason, shadowed, own };
}
...
function loadEnvFor(cwd = process.cwd(), { noProfile = false, shellRcEnv = {} } = {}) {
  ...
  return loadEnvForProfile(profile, { shellRcEnv, reason });
}
