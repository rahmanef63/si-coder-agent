...
// ---------------------------------------------------------------------------
// Service logging is injectable so tests/agent callers can keep machine streams clean while
// human CLI callers retain the existing progress output. Missing methods are deliberate no-ops.
...
async function resolveZone(fullDomain, cloudflareToken, { timeoutMs = CLOUDFLARE_TIMEOUT_MS, zoneId, logger } = {}) {
  const output = normalizeLogger(logger);
  ...
  if (pinned) {
    ...
    output.log(`ℹ️ using pinned zone ${zid} for '${fqdn}' (zone lookup skipped)`);
...
async function configureDnsRecord({
  ...
  comment = 'managed by si-coder /sc-cf', logger,
} = {}) {
  const output = normalizeLogger(logger);
  ...
  try {
    ...
    const r = await resolveZone(fullDomain, token, { timeoutMs, zoneId, logger: output });
    if (r.error) { output.log(`⚠️ ${r.error}`); return { skipped: true, reason: r.error }; }
    ...
    if (matches.length === 1) {
      ...
      if (sameContent(existing.content, target)
        ...
        output.log(`✅ ${recType} record for ${fullDomain} already correct`);
        return { skipped: false, alreadyExists: true };
      }
      output.log(`📝 PATCH ${recType} '${recordName}' -> ${target} (record ${existing.id})...`);
      await updateRecord({
      ...
      output.log(`✅ Cloudflare DNS updated for ${fullDomain}`);
    ...
    for (const clashType of clashTypes) {
      ...
      output.log(`🧹 removing clashing ${clashType} '${recordName}' (${doomed.id}) before creating ${recType}`);
      removed.push({
    ...
    output.log(`📝 POST ${recType} '${recordName}' -> ${target} in zone ${zoneName || zid}...`);
    try {
      await createRecord({
      ...
      if (e.cfCode === CF_ERR.RECORD_EXISTS) {
        output.log(`✅ ${recType} record for ${fullDomain} already exists (created concurrently)`);
        return { skipped: false, alreadyExists: true };
      }
      ...
      if (removed.length > 0) {
        ...
        for (const snap of removed) {
          try {
            output.warn(`↩️ create failed — restoring the ${snap.type} '${snap.name}' that was removed for it`);
            await createRecord({
            ...
            restored = false;
            output.warn(`⚠️ restore of ${snap.type} '${snap.name}' failed: ${restoreErr.message}`);
          }
        }
        output.warn(`⚠️ Cloudflare DNS error: ${e.message}`);
        if (!restored) {
          output.warn(`⚠️ ${recordName} may now have NO record — re-create it from priorRecords in the result.`);
        }
        return {
...
async function configureDns({
  ...
  proxied = false, ttl = 1, zoneId, timeoutMs = CLOUDFLARE_TIMEOUT_MS, settleMs = DNS_SETTLE_MS, logger,
} = {}) {
  const output = normalizeLogger(logger);
  const token = resolveToken(cloudflareToken || apiToken);
  if (!token || !fullDomain) {
  ...
  try {
    ...
    return await configureDnsRecord({
      ...
      proxied, ttl, zoneId, timeoutMs, settleMs, logger: output,
    });
  } catch (e) {
    output.warn(`⚠️ Cloudflare DNS error: ${e.message}`);
    return { skipped: true, reason: e.message };
...
function makeClient({ apiToken, cloudflareToken, accountId, timeoutMs = CLOUDFLARE_TIMEOUT_MS, logger } = {}) {
  const tokenOf = () => {
  ...
  const withSoftToken = (fn) => async (args = {}) =>
    fn({ timeoutMs, logger, ...args, cloudflareToken: resolveToken(cloudflareToken || apiToken) });

  return {
    call,
    accountId,
    verifyToken,
    resolveZone: (fullDomain, opts = {}) => resolveZone(fullDomain, tokenOf(), { timeoutMs, logger, ...opts }),
