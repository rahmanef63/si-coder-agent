...
The Agent Skills standard defines a skill as a **directory containing `SKILL.md`**. For local clients, this is the preferred source format. The current SI-Coder CLI/installer requires **Node.js 22, 24, or 26**.

## Canonical source

- [skills/sc directory](https://github.com/rahmanef63/si-coder-agent/tree/v0.9.8/skills/sc)
- [raw `SKILL.md`](https://raw.githubusercontent.com/rahmanef63/si-coder-agent/v0.9.8/skills/sc/SKILL.md)
...
The installer reads `skills/catalog.json` and installs only **active/default** skills into the selected user skills location. Stub and legacy skills remain in the repository but are not installed/routed by default. It does not copy provider credentials. If `npm link` cannot expose the global `sc` command, the installer reports that failure instead of hiding it.
...
- [ZIP package](https://github.com/rahmanef63/si-coder-agent/releases/download/v0.9.8/sc.zip)
- [optional `.skill` archive](https://github.com/rahmanef63/si-coder-agent/releases/download/v0.9.8/sc.skill)
