...
const BUILTIN_PROVIDERS = [
...

  {
    ...
    vars: [
      { key: 'CONVEX_DEPLOY_KEY', required: true, secret: true,
        ...
        navigation: ['Select the Convex project', 'Open the production deployment', 'Settings', 'General', 'Generate Production Deploy Key', 'Copy the key'],
  ...
  },
  {
    ...
    async check(env) {
      if (!env.COMPOSIO_API_KEY) return skip('COMPOSIO_API_KEY not set');
      const r = await probe('https://backend.composio.dev/api/v3.1/tools?limit=1', {
        headers: { 'x-api-key': env.COMPOSIO_API_KEY },
      });
      if (r.status === 401 || r.status === 403) return bad(`API key rejected (${r.status})`);
      if (!r.ok) return bad(`HTTP ${r.status}${r.error ? ` (${r.error})` : ''}`);
