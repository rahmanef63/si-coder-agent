...
function renderFinderFrame({
  ...
  const activityLines = activity.length ? Math.min(6, activity.length) + 2 : 0;
  const bodyHeight = Math.max(5, height - lines.length - activityLines - 4);
  ...
  if (activity.length) {
    lines.push(`${DIM}${fit('  LAST ACTION', width, { pad: false })}${RESET}`);
    for (const line of activity.slice(-Math.min(6, activity.length))) {
...
function selectFinderFrame({
  ...
  return new Promise(resolve => {
    ...
    const render = () => renderFinderFrame({
      ...
      activeItems: visible(), cursor, query, sections, activity, output,
    });
