...
/**
 * Persistent-layer menu picker used by bare `sc`.
 *
 * Returns one of:
 *   { type: 'open', id }   — descend into a branch
 *   { type: 'select', id } — run/submit a leaf action
 *   { type: 'back' }       — caller pops one breadcrumb layer
 *   { type: 'quit' }       — caller ends the interactive session
 *
 * Key contract:
 *   ↑/↓ move · Tab open deeper · →/Enter open/run · ←/Esc back · Ctrl-D quit
 */
function selectLayer(title, breadcrumb, items, { canBack = true } = {}) {
  return new Promise(resolve => {
    const input = process.stdin;
    const output = process.stdout;
    ...
    const visible = () => {
      ...
      const q = query.toLowerCase();
      return items.filter(it => `${it.id} ${it.label} ${it.hint || ''}`.toLowerCase().includes(q));
    };

    const render = () => {
      const list = visible();
      if (cursor >= list.length) cursor = Math.max(0, list.length - 1);
      if (printed) output.write(`${ESC}[${printed}A`);
      output.write(`${ESC}[0J`);
      const lines = [title];
      lines.push(`  ${ESC}[2m${breadcrumb.join('  ›  ')}${ESC}[0m`);
      lines.push(`  search: ${query}${ESC}[4m ${ESC}[0m${query ? '' : '  (type to filter)'}`);
      lines.push(`  ↑/↓ move · Tab deeper · →/Enter open/run${canBack ? ' · ←/Esc back' : ''} · Ctrl-D quit`);
      lines.push('');
      if (!list.length) lines.push(`  ${ESC}[2mno match for "${query}" — Esc clears search${ESC}[0m`);
      list.forEach((it, i) => {
        const here = i === cursor;
        const kind = it.kind === 'branch' ? '›' : '•';
        const pointer = here ? '❯ ' : '  ';
        const label = `${kind} ${it.label}${it.hint ? `  — ${it.hint}` : ''}`;
        lines.push(truncate(here ? `${ESC}[7m${pointer}${label}${ESC}[0m` : `${pointer}${label}`));
      });
      output.write(lines.map(l => `${l}\n`).join(''));
      printed = lines.length;
    };
    ...
    const onData = d => {
      ...
      for (let i = 0; i < str.length; i++) {
        ...
        if (rest.startsWith(UP))   { cursor = list.length ? (cursor - 1 + list.length) % list.length : 0; i += 2; continue; }
        if (rest.startsWith(DOWN)) { cursor = list.length ? (cursor + 1) % list.length : 0; i += 2; continue; }
        if (rest.startsWith(RIGHT)) { i += 2; render(); return activate(list[cursor]); }
        if (rest.startsWith(LEFT))  { i += 2; render(); return finish({ type: 'back' }); }

        const ch = str[i];
        const code = ch.charCodeAt(0);
        if (code === 9) { // Tab only descends; it never accidentally submits a leaf action.
          const item = list[cursor];
          if (item?.kind === 'branch') { render(); return finish({ type: 'open', id: item.id }); }
        } else if (code === CR || code === LF) {
          render(); return activate(list[cursor]);
        } else if (code === ETX) {
          showCursor(); output.write('\n'); process.exit(130);
        } else if (code === EOT) {
          render(); return finish({ type: 'quit' });
        } else if (code === BS || code === DEL) {
          query = query.slice(0, -1); cursor = 0;
        } else if (code === 27) {
          if (query) { query = ''; cursor = 0; }
          else { render(); return finish({ type: 'back' }); }
...
module.exports = {
  askVisible, askHidden, redactValue, isInteractive, confirm, selectOne, selectMany, selectLayer,
};
