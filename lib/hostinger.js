...
// Bound every Hostinger API call so a hung connection can't stall a zero-human run.
// Mirrors the AbortController pattern in lib/convex.js probeBackend.
const HOSTINGER_TIMEOUT_MS = 15000;
...
async function fetchWithTimeout(url, init = {}, timeoutMs = HOSTINGER_TIMEOUT_MS) {
...
async function resolveRoot(fullDomain, hostingerToken) {
  const res = await fetchWithTimeout(PORTFOLIO_URL, {
...
async function configureDnsRecord({ fullDomain, type, target, hostingerToken }) {
  ...
  try {
    ...
    const zoneRes = await fetchWithTimeout(zoneUrl(rootDomain), {
      headers: { Authorization: `Bearer ${hostingerToken}`, Accept: 'application/json' },
    });
    if (!zoneRes.ok) return { skipped: true, reason: `zone ${zoneRes.status}` };
    const zoneRecords = zoneRes.json();
    ...
    const putRes = await fetchWithTimeout(zoneUrl(rootDomain), {
      method: 'PUT',
