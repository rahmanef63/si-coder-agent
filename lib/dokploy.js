...
function makeClient({ apiUrl, apiKey, timeoutMs = 15000 }) {
  ...
  // SCD-RES-1: a single fetch leaves transient failures (rate limits, backend restarts,
  // dropped connections) to abort a zero-human run. Retry up to 3 attempts with exponential
  // backoff on 429, 5xx, and network/abort errors. Non-429 4xx are deterministic client
  // errors — tag them isHttpError and rethrow immediately, no retry. Timeout behavior is
  // preserved: each attempt arms its own abort timer covering the body read.
  ...
  async function call(endpoint, method = 'GET', body = null) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        if (!res.ok) {
          // Retry on rate limit + server errors; treat the rest of 4xx as terminal.
          if (res.status === 429 || res.status >= 500) {
            const err = new Error(`Dokploy ${res.status} ${endpoint}: ${typeof data === 'object' ? JSON.stringify(data) : data}`);
        ...
        lastErr = e.name === 'AbortError'
          ? new Error(`Dokploy timeout after ${timeoutMs}ms ${endpoint}`)
          : e;
