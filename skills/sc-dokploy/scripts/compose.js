...
async function findCompose(dokploy, name) {
  const projects = await dokploy.listProjects();
  for (const p of projects) {
    const c = p.environments?.[0]?.compose?.find(x => x.name === name);
    if (c) return { project: p, compose: c };
  }
  throw new Error(`compose '${name}' not found in any project`);
}
...
async function main() {
  ...
  if (cmd === 'show') {
    if (!args.compose) { console.error('Usage: compose.js show --compose <name>'); process.exit(1); }
    const { compose } = await findCompose(dokploy, args.compose);
    const full = await dokploy.getCompose(compose.composeId);
    console.log(JSON.stringify(full, null, 2));
    return;
  }
  if (cmd === 'env') {
    if (!args.compose) { console.error('Usage: compose.js env --compose <name>'); process.exit(1); }
    const { compose } = await findCompose(dokploy, args.compose);
    const full = await dokploy.getCompose(compose.composeId);
  ...
  if (cmd === 'deploy') {
    if (!args.compose) { console.error('Usage: compose.js deploy --compose <name>'); process.exit(1); }
    const { compose } = await findCompose(dokploy, args.compose);
