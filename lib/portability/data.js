...
function plan(bundle,options,state){
  ...
  for(const u of bundle.users){
    ...
    for(const c of u.connections){
      ...
      if(provider)try{const mapped=c.provider==='hostinger'?(c.authMethod==='direct'?'api-token':c.authMethod==='mail'?'mail-api-token':c.authMethod):(c.authMethod==='provider-oauth'?'dcr-oauth':c.authMethod);method=C.authOption(provider,source,mapped)}catch{}
