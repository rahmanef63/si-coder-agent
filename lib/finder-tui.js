...
function renderFinderFrame({
  ...
  // three columns from resizing from 1/3 -> 1/4 when the user opens Providers. On deeper
  // paths, the oldest column slides out and the newest one occupies the same fourth slot.
  const slotCount = width >= 132 ? 4 : width >= 92 ? 3 : 2;
  const visibleColumns = columns.slice(-Math.max(1, Math.min(slotCount, columns.length)));
  ...
  // Footer height is fixed: separator + INFO + PREVIEW/RESULT label + four detail rows.
  // RESULT temporarily replaces PREVIEW instead of expanding the screen vertically.
  const footerRows = 7;
  const bodyHeight = Math.max(5, height - lines.length - footerRows);
  ...
  const footerBody = activity.length
    ? activity.slice(-4).map(stripAnsi)
    : preview.slice(0, 4);
  ...
  // Never emit a newline on the terminal's final row. A trailing newline at row=height can
  // scroll an alternate screen and make the Finder header appear to vanish after redraws.
  const frame = lines.slice(0, height);
  while (frame.length < height) frame.push('');
  output.write(frame.map(line => `${line}${RESET}`).join('\n'));
}
