...
function loadEnvForProfile(name, { shellRcEnv = {}, reason = 'explicit user selection', connectionOverrides = {} } = {}) {
  ...
  // credential set. This is the isolation boundary that lets one user keep work/personal or
  // multiple project credentials without merging fields from different accounts.
  for (const provider of PROVIDERS) {
    ...
    const values = C.readValues(name, provider.id, selected.id);
    Object.assign(own, values);
    selectedConnections[provider.id] = {
      id: selected.id, label: selected.label, authMethod: selected.authMethod, scope: selected.scope,
