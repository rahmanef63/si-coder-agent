...
function upsertScMetadata(text, fields = {}) {
  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    const pattern = new RegExp(`^\\s{2,}${fullKey.replace(/[.*+?^${}()|[\\]\\]/g, '\\$&')}:`);
...
function ensureAutoMetadata(text, options = {}) {
  ...
  const tags = options.tags?.length ? options.tags : deriveTags({ name, description, useWhen, body });
  const aliases = options.aliases?.length ? options.aliases : deriveAliases(tags, { name });
  return upsertScMetadata(temp, {
    tags,
    aliases,
    'tags-source': options.tags?.length || options.aliases?.length ? 'manual' : AUTO_TAG_VERSION,
  });
}
