...
function normalizeMeta(doc) {
  ...
  for (const user of Object.values(out.users || {})) {
    ...
    for (const [providerId, node] of Object.entries(user.providers || {})) {
      node.connections ||= {};
      for (const meta of Object.values(node.connections || {})) {
        ...
        if (providerId === 'github' && meta.source === 'sc' && ['personal-access-token', 'fine-grained-pat'].includes(meta.authMethod)) {
          // SC direct GitHub now has one explicit contract: PAT classic. Existing metadata is
          // normalized to that contract; a stored fine-grained token then fails provider
          // validation and the UI guides the user to rotate it without ever reading it out.
          meta.authMethod = 'classic-pat';
        }
...
for(const name of ["writeMeta", "migrateMetadata", "writeValues", "removeValues", "create", "setDefault", "setLabel", "setExternal", "remove", "deleteUser", "duplicateUser", "renameUser", "migrateLegacy"]) {const fn=module.exports[name];module.exports[name]=(...args)=>withStateLock(()=>fn(...args));}
