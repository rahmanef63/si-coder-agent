...
async function startCredentialHub({user=null,providerId=null,connectionId=null,authMethod=null,port=0,ttlMs=600000,check=null,authorize=CC.authorize,sync=CC.sync}={}) {
  ...
  const expiresAt=Date.now()+Math.min(Math.max(Number(ttlMs)||600000,1),600000),forms=new Map();let server,timer;
  ...
  async function handle(req,res){
    ...
    if(req.headers.host!==new URL(origin).host||(req.headers.origin&&req.headers.origin!==origin))return reply(res,403,{error:'origin-not-allowed'});
    if(req.method==='GET'&&req.url==='/'){res.writeHead(200,{...headers,'Content-Type':'text/html; charset=utf-8','Content-Security-Policy':`default-src 'none'; script-src 'nonce-${nonce}'; style-src 'nonce-${nonce}'; connect-src 'self'; form-action 'none'; base-uri 'none'; frame-ancestors 'none'`});res.end(renderHub(nonce));return;}
    if(!auth(req))return reply(res,401,{error:'session-expired'});
    if(req.method==='GET'&&req.url==='/api/catalog')return reply(res,200,{...catalog(),initial:{user,providerId,connectionId,authMethod},expiresAt});
    if(req.method==='POST'&&req.url==='/api/transfer'){if(req.headers.origin!==origin)return reply(res,403,{error:'origin-required'});if(++mutations>100)return reply(res,429,{error:'rate-limited'});return require('./portability/http').transfer(req,res,{authorized:true,origin});}
    if(req.method!=='POST'||req.url!=='/api/action')return reply(res,404,{error:'not-found'});
  ...
  return{server,url:`http://127.0.0.1:${server.address().port}/#${token}`,origin:`http://127.0.0.1:${server.address().port}`,close};
}
