...
function repoInfo(repoDir = DEFAULT_REPO) {
  ...
  const dirty = Boolean(git(repoDir, ['status', '--porcelain']).stdout);
  return { gitCheckout: true, repoDir, branch, head, dirty };
}
...
function performUpdate(options = {}) {
  ...
  if (status.dirty) throw new Error('sc update refuses a dirty checkout; commit/stash your changes first');
...
module.exports = { DEFAULT_REPO, git, repoInfo, checkUpdate, performUpdate };
