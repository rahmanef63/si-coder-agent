...
function printRows(rows, options = {}) {
  if (options.json) {
    const payload = options.registryEnvelope
      ? {
          version: 1,
          source: 'si-coder',
      ...
      : { query: options.query || null, count: rows.length, skills: rows };
...
function listOrSearch(argv = [], options = {}) {
  ...
  printRows(rows, { query, json: has('--json', argv), registryEnvelope: Boolean(options.registryEnvelope) });
  return rows;
}
...
async function handleSkill(argv = [], fallback) {
  const sub = argv[0];
  if (sub === 'list') return listOrSearch(argv.slice(1), { registryEnvelope: true });
  if (sub === 'search' || sub === 'find') return listOrSearch(argv.slice(1), { registryEnvelope: true });
...
module.exports = { REGISTRY_CONTRACT, has, value, listValue, newContract, printRows, listOrSearch, createNew, handleSkill, runStandalone };
