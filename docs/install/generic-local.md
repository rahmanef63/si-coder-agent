...
- [skills/sc directory](https://github.com/rahmanef63/si-coder-agent/tree/v0.8.12/skills/sc)
...
SI-Coder exposes one stdio MCP server at `scripts/sc-mcp.js`. Its `tools/list` comes from `.mso/functions.json`, the same manifest MSO uses. See [`../tool-calling.md`](../tool-calling.md) for explicit user-scoped tool names and manual registration examples.

## Archive-only clients

Only when a client explicitly asks for an archive/package:

- [ZIP package](https://github.com/rahmanef63/si-coder-agent/releases/download/v0.8.12/sc.zip)
- [optional `.skill` archive](https://github.com/rahmanef63/si-coder-agent/releases/download/v0.8.12/sc.skill)
