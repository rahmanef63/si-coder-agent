...
function userCredentialCard({ provider, createAt, createCommand, navigation = [], note, saveWith, saveDestination, continueWith, hosted = false } = {}) {
  ...
  const hasUrl = Boolean(createAt);
  const hasCommand = Boolean(createCommand);
  return {
    title: `Give SI-Coder access to ${name}`,
    message: hasUrl
      ? 'Open the official page, follow the navigation path below, then store the value through SI-Coder. The value will not be displayed back to you.'
      : hasCommand
        ? 'Get or generate the value locally with the command below, then store it through SI-Coder. The value will not be displayed back to you.'
        : 'Follow the source instructions below, then store the value through SI-Coder. The value will not be displayed back to you.',
    primaryAction: hasUrl ? { label: 'Open the official page', url: createAt } : null,
    getWith: createCommand || null,
    navigation: Array.isArray(navigation) ? navigation : [],
    navigationText: Array.isArray(navigation) && navigation.length ? navigation.join(' → ') : null,
