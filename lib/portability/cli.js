...
async function cli(args,{askHidden,isInteractive}={}){
  const action=args._[1],allowed=['_','out','file','user','include-secrets','apply','confirm','prefix','policy','accept-warnings','help'];
  if(Object.keys(args).some(k=>!allowed.includes(k)))throw Error('unknown_data_flag: passphrases cannot be supplied as arguments');
  if(args.help===true||!['export','import'].includes(action)){console.log('Credential portability (not the same as `sc user import`, which only imports legacy shell values):\n\n1) Export metadata only:\n   sc data export --user USER --out backup.integration-bundle.json\n\n2) Export encrypted direct credentials (local interactive terminal only; passphrase is prompted twice):\n   sc data export --user USER --include-secrets --out backup.integration-bundle.enc.json\n\n3) Preview an import (no changes yet):\n   sc data import --file FILE.json [--prefix imported-] [--policy skip|error]\n\n4) Apply the exact preview using the returned planId and the same options:\n   sc data import --file FILE.json [--prefix imported-] [--policy skip|error] --apply --confirm PREVIEW_ID [--accept-warnings]\n\nEncrypted imports prompt for the passphrase again. Passphrases cannot be supplied in argv/chat/tool JSON.\nIf you see unsafe_store_path, every SI-Coder store directory must be owner-only (700) and files 600.\nUse the browser hub for file upload/download: sc setup --web');return;}
  let passphrase;
