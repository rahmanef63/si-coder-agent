...
function repositoryVerifyAction(input = {}, options = {}) {
  ...
  const regression = syntax.ok ? Repo.runRegression(root) : { code: 1, stdout: '', stderr: 'skipped because syntax failed' };
  ...
  const secretScan = Repo.scanAgentState(root);
  const ok = syntax.ok && regression.code === 0 && docs.code === 0 && skills.ok && secretScan.ok;
  ...
  return {
    ...
    secretScan: { ok: secretScan.ok, filesScanned: secretScan.filesScanned, findingCount: secretScan.findings.length },
